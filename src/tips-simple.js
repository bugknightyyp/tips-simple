define(function(require, exports, module){
  ...
  var tips = function(el, options ){
      ...
      var options = $.extend({
        msg: "",// prompt message
        position: "right", //the position tips wrapper relate the  text input , the value must is the one of (top, right, bottom, left), the default value is left
        vernier: "", // setting position on the basis of  default setting
        closeAble: true,//if show the button on close. default show
        tipsTimeout: "infinite", //tips auto hide delay time
        left: 0,
        top: 0,
        width: 200 // the width of tips wraper
      }, options || {});
      
      var tmpl = "<div style=\"width:"+ options.width +"px;\" class=\"tips-simple-box\">"
                +   "<div nam=\"arrow\" class=\"arrow-${ arrow-position }\">&nbsp;</div>"
                +   (options.closeAble ? "<div name=\"close\" class=\"close-${ close-position }\">&nbsp;</div>" : "")
                +   "<div name=\"content\" class=\"tips-content\">${ msg }</div>"
                +"</div>",
          timer = null;
      ...
      */
      var build = function(){
        ...
        this.panel = tmpl; //save  tips panel
      };
